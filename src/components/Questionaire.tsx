...
export default function Questionaire() {
    ...
    const [currentQuestionId, setCurrentQuestionId] = useState<number | null>(2);
      ...
      <div className="flex flex-col gap-4 bg-[#FFFFFF] p-6 rounded-lg max-w-[700px]">
        {currentQuestion && (
            <div className="flex flex-col gap-4">
                <h1 className="text-2xl font-bold text-[#1E0F62]">{currentQuestion.question}</h1>
                <div className="flex flex-col gap-4">
                    {currentQuestion.options && Object.entries(currentQuestion.options).map(([option, optionId]) => (
                        <button key={optionId} onClick={() => handleOptionClick(optionId)} className="bg-[#1E0F62] text-white text-[20px] font-bold w-full max-w-[700px] px-4 py-2 rounded-2xl shadow-lg">{option}</button>
                    ))}
                </div>
            </div>
        )}
